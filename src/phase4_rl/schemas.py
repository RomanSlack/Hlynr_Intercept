...
class NormalizationInfo(BaseModel):
    ...
    transform_version: str = Field(..., description="Coordinate transform version")
...
class HealthResponse(BaseModel):
    ...
    vecnorm_stats_loaded: Optional[str] = Field(None, description="Loaded VecNormalize stats ID")
...
def check_if_clamped(original: RateCommand, clamped: RateCommand) -> tuple[bool, str]:
    """Check if rate commands were clamped and return reason."""
    reasons = []
    
    if original.pitch != clamped.pitch:
        reasons.append(f"pitch {original.pitch:.3f} -> {clamped.pitch:.3f}")
    if original.yaw != clamped.yaw:
        reasons.append(f"yaw {original.yaw:.3f} -> {clamped.yaw:.3f}")
    if original.roll != clamped.roll:
        reasons.append(f"roll {original.roll:.3f} -> {clamped.roll:.3f}")
    
    if reasons:
        return True, f"Rate limits: {', '.join(reasons)}"
    return False, ""
